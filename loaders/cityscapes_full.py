...
class CityscapesDataset(data.Dataset):
	...
	def __init__(self, im_size=cityscapes.shape, mode="val"):
		img_fns_dense = glob.glob(os.path.join(paths["cityscapes_path"], "images", mode, "**", "*.png"), recursive=True)
		lbl_fns_dense = [fn.replace("images", "annotations").replace("leftImg8bit", "gtFine_labelIds") for fn in img_fns_dense]

		img_fns_coarse = glob.glob(os.path.join(paths["cityscapes_coarse_path"], "images", mode, "**", "*.png"), recursive=True)
		lbl_fns_coarse = [fn.replace("images", "annotations").replace("leftImg8bit", "gtFine_labelIds") for fn in img_fns_dense]

		self.image_fnlist = img_fns_dense + img_fns_coarse
		self.label_fnlist = lbl_fns_dense + lbl_fns_coarse
		...
		class_weights = -1 * np.log(np.array(class_freq))
